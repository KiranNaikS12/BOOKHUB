function validateInput(inputElement, errorMessageElement, validationRegex, minLength, emptyErrorMessage, invalidErrorMessage, consecutiveSameDigitsErrorMessage) {
    ...
    input.addEventListener('blur', function () {
        ...
        if (inputValue === '') {
            ...
            error.innerHTML = `<small style="color: red;">${emptyErrorMessage}</small>`;
        } else if (inputValue.length < minLength || !validationRegex.test(inputValue)) {
            input.classList.add('invalid');
            error.innerHTML = `<small style="color: red;">${invalidErrorMessage}</small>`;
        } else if (hasConsecutiveSameDigits(inputValue)) {
            input.classList.add('invalid');
            error.innerHTML = `<small style="color: red;">${consecutiveSameDigitsErrorMessage}</small>`;
        } else {
            error.innerHTML = '';
        }
    });
...
function validatePasswordConfirmation(passwordElement, confirmationElement, errorMessageElement, emptyErrorMessage, mismatchErrorMessage) {
    ...
    confirmationInput.addEventListener('blur', function () {
        ...
        if (confirmationValue === '') {
            ...
            confirmationInput.classList.add('invalid');
            error.innerHTML = `<small style="color: red;">${mismatchErrorMessage}</small>`;
        } else {
            error.innerHTML = '';
        }
    });
...
// Adding validation checks for confirmation password
...
function validateCheckbox(checkboxElement, errorMessageElement, uncheckedErrorMessage) {
        ...
        checkbox.addEventListener('change', function () {
            if (!checkbox.checked) {
                error.innerHTML = `<small style="color: red;">${uncheckedErrorMessage}</small>`;
            } else {
                error.innerHTML = '';
            }
        });
...
document.getElementById('registrationForm').addEventListener('submit', function (event) {
    ...
    if (!checkbox.checked) {
        checkboxError.innerHTML = `<small style="color: red;">Please agree to the terms and conditions.</small>`;
